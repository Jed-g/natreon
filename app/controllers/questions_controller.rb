class QuestionsController < ApplicationController
  ...
  def submit_question
    ...
    return render json: { message: 'Bad request' }, status: 400 if question.nil? || question.length == 0

    question_object = Question.new
    question_object.question = question

    return render json: { message: 'Bad request' }, status: 400 unless question_object.valid?
  ...
  def upvote_question
    ...
    return render json: { message: 'Bad request' }, status: 400 if question.nil?

    session[:questions_upvoted] = [] if session[:questions_upvoted].nil?

    return render json: { message: 'Already upvoted' }, status: 400 if session[:questions_upvoted].include?(id)
    ...
    return render json: { message: 'Bad request' }, status: 400 unless question.valid?
  ...
  def downvote_question
    ...
    return render json: { message: 'Already downvoted' }, status: 400 if session[:questions_downvoted].include?(id)
  ...
  def cancel_upvote_question
    ...
    unless session[:questions_upvoted].include?(id)
      return render json: { message: 'Nothing to cancel, question hasn\'t been upvoted' }, status: 400
    end
  ...
  def cancel_downvote_question
    ...
    unless session[:questions_downvoted].include?(id)
      return render json: { message: 'Nothing to cancel, question hasn\'t been downvoted' }, status: 400
    end
