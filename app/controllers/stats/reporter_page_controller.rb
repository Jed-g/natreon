...
module Stats
  class ReporterPageController < ApplicationController
    ...
    def register_new_page_visit
      ...
      page_visit = ReporterPageVisit.new(session_id: session.id, ip_address: request.remote_ip)
    ...
    def register_new_page_visit_with_ip_param
      ...
      page_visit = ReporterPageVisit.new(session_id: session.id, ip_address: ip)
      return render_internal_server_error unless page_visit.valid?
    ...
    def update_page_visit
      ...
      return render_bad_request unless ReporterPageVisit.exists?(id: visit_id)

      page_visit = ReporterPageVisit.find(visit_id)
