<script lang="ts">
	...
	onMount(() => {
		progressBarValue = Math.round(
			((totalPoints - inProgressTotalPoints.previous_threshold) * 100) /
				(inProgressTotalPoints.threshold - inProgressTotalPoints.previous_threshold)
		);
...
<Card.Root>
	<Card.Header class="p-2">
		<Card.Title class="text-xs">Total Points: {totalPoints}</Card.Title>
		<Card.Description>
			<div class="flex flex-col space-y-1 items-center self-stretch">
				<div class="shrink-0">
					{#if !badgeIconError}
						<img
							src={Badges[inProgressTotalPoints.badge]}
				...
				<p class="text-sm font-medium leading-none">{inProgressTotalPoints.badge}</p>
				<div class="flex self-stretch gap-2 items-center">
					<p class="text-sm font-medium leading-none text-muted-foreground">
						{inProgressTotalPoints.previous_threshold}
					</p>
					<progress
					...
					/>
					<p class="text-sm font-medium leading-none text-muted-foreground">
						{inProgressTotalPoints.threshold}
					</p>
				</div>
			</div>
		</Card.Description>
