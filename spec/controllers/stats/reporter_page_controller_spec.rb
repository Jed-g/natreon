  ...
  describe "#register_new_page_visit" do
    context "with valid geolocation" do
      ...
      it "registers a new page visit and returns a success message" do
        ...
        created_page_visit = ReporterPageVisit.find(session[:visit_id])
    ...
    context "when ReporterPageVisit is not valid" do
      let(:valid_coords) { {lat: 23.456, lon: 45.678} }

      before do
        allow(controller).to receive(:geolocation_from_ip).with(request.remote_ip).and_return(valid_coords)
        allow_any_instance_of(ReporterPageVisit).to receive(:valid?).and_return(false)
      end
  ...
  describe "#update_page_visit" do
    ...
    context "with valid parameters" do
      before do
        ...
        create(:reporter_page_visit, id: valid_visit_id, session_id: session.id)
      end
      ...
      it "updates the page visit and returns a success message" do
        ...
        updated_page_visit = ReporterPageVisit.find(valid_visit_id)
    ...
    context "when ReporterPageVisit does not exist" do
      before do
        session[:visit_id] = valid_visit_id
        allow(ReporterPageVisit).to receive(:exists?).with(id: valid_visit_id).and_return(false)
      end
